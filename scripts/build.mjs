...
const fingerprint = async name => createHash('sha256').update(await readFile(join(root, 'site/assets', name))).digest('hex').slice(0, 12)
const styleName = `style.${await fingerprint('style.css')}.css`
const scriptName = `app.${await fingerprint('app.js')}.js`
...
const projectCards = projects.map((p,i) => {
 ...
 <div class="project-plate plate-${group.id}" data-scene aria-hidden="true"><span class="plate-number">${number}</span><span class="plate-orbit"></span><span class="plate-axis"></span>${artwork(p.name)}${icon(group.icon,'plate-mark')}<span class="plate-note">${e(group.name)}</span><div class="plate-corner">↗</div></div>
...
<script>(()=>{const r=document.documentElement;let t,m;try{t=localStorage.getItem('field-theme');m=localStorage.getItem('field-motion')}catch{}r.dataset.theme=t==='day'||t==='night'?t:matchMedia('(prefers-color-scheme: dark)').matches?'night':'day';r.dataset.motion=m==='off'||matchMedia('(prefers-reduced-motion: reduce)').matches?'off':'on'})()</script>
<link rel="stylesheet" href="assets/${styleName}"><script type="application/ld+json">${JSON.stringify(graph).replaceAll('<','\\u003c')}</script><script src="assets/${scriptName}" defer></script>
</head><body>
<a class="skip" href="#collection">Skip to the projects</a>
<header class="masthead" id="top"><a class="wordmark" href="#top" aria-label="App Automaton field guide home">${icon('flower')}<span>APP<br>AUTOMATON</span></a><span class="masthead-note">A FIELD GUIDE<br>BY RENOCRYPT</span><nav aria-label="Main"><a href="#collection">The collection <sup>${projects.length}</sup></a><a href="#colophon">Our intention</a><div class="view-controls"><button class="theme-toggle view-toggle" type="button" aria-label="Switch to night palette" hidden>${icon('sun','sun')}${icon('moon','moon')}<span class="theme-label">Night</span></button><button class="motion-toggle view-toggle" type="button" aria-label="Motion" aria-pressed="true" hidden><svg class="motion-symbol" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path class="motion-pause" d="M9 6v12m6-12v12"/><path class="motion-play" d="m9 5 10 7-10 7Z"/></svg><span class="motion-label">Motion</span></button></div></nav></header>
<main>
<section class="hero" data-scene aria-labelledby="hero-title"><div class="hero-kicker"><span>INDEPENDENT TOOLS. SHARED POSSIBILITIES.</span><span>VOLUME 001 <span class="tiny-star">✳</span></span></div>
<div class="hero-stage"><div class="hero-aside"><span class="hero-coordinate" aria-hidden="true">A STUDY IN<br>POSSIBILITY</span><button type="button" class="arrange-toggle" aria-label="Turn a new leaf: rearrange the decorative flower" hidden><span>Turn a<br>new leaf</span>${icon('arrow')}</button><span class="arrangement-status sr-only" aria-live="polite"></span></div><h1 id="hero-title"><span class="hero-open">Open</span> <span class="hero-nature">by nature.</span></h1><div class="hero-object">${bloom()}</div></div>
...
<div class="ribbon" aria-hidden="true" data-scene><div class="ribbon-track">${Array.from({length:2},()=>`<div class="ribbon-set"><span>Made to be explored</span>${icon('spark')}<span>Open to possibility</span>${icon('flower')}</div>`).join('')}</div></div>
<section class="intro" data-scene aria-labelledby="intro-title"><span class="section-label">01 / THE IDEA</span><div><h2 id="intro-title">Good tools make<br>room for <em>what if.<svg class="idea-underline" viewBox="0 0 200 18" fill="none" aria-hidden="true"><path d="M3 10C49 1 139 1 195 7M23 15c47-7 102-7 137-3" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></em></h2><p>Words become working code. Sound finds a voice. Images open into three dimensions. This is a collection of public projects from App Automaton, brought together by RenoCrypt for the pleasure of finding something useful.</p><a class="text-link" href="${e(config.mainWebsite)}">Meet the App Automaton workshop ${icon('arrow')}</a></div><div class="intro-drawing" aria-hidden="true">${icon('orbit')}<span>ALWAYS A WORK IN PROGRESS</span></div></section>
...
<section class="colophon" data-scene id="colophon" aria-labelledby="colophon-title"><span class="section-label">03 / A NOTE FROM RENOCRYPT</span><div class="colophon-grid"><h2 id="colophon-title">A living index.<br>A human<br><span>point of view.</span></h2><div class="colophon-copy"><p>We like tools that invite you in. Things you can inspect, understand, and make your own.</p><p>Every entry here is a public App Automaton repository with a website in its GitHub About section. The collection refreshes each week. The projects, and the people behind them, supply the substance.</p><p>RenoCrypt gives the collection a different setting. App Automaton is where the work lives.</p><a class="text-link" href="${e(config.mainWebsite)}">Explore App Automaton ${icon('arrow')}</a><div class="colophon-mark" aria-hidden="true">${icon('spark')}</div></div></div></section>
...
await cp(join(root,'site/assets/style.css'), join(out,'assets',styleName))
await cp(join(root,'site/assets/app.js'), join(out,'assets',scriptName))
